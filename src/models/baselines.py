...
class IsolationForestBaseline:  
    ...
    def evaluate(self, X, true_labels):  
        ...
        return {  
            ...
            "roc_auc": roc_auc_score(true_labels, scores) if true_labels.sum() > 0 else 0.0,  
        }
...
class SARIMABaseline:  
    ...
    def evaluate(self, true_labels):  
        scores, pred_labels = self.predict()  
        min_len = min(len(true_labels), len(pred_labels))  
        return {  
            "precision": precision_score(true_labels[:min_len], pred_labels[:min_len], zero_division=0),  
            "recall": recall_score(true_labels[:min_len], pred_labels[:min_len], zero_division=0),  
            "f1_score": f1_score(true_labels[:min_len], pred_labels[:min_len], zero_division=0),  
            "roc_auc": roc_auc_score(true_labels[:min_len], scores[:min_len]) if true_labels[:min_len].sum() > 0 else 0.0,  
        }  
