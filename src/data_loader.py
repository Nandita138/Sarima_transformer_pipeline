...
class SyntheticFeederGenerator:  
    ...
    def _inject_anomalies(self, df: pd.DataFrame) -> pd.DataFrame:  
        ...
        for idx in anomaly_indices:  
            ...
            if atype == "spike":  
                ...
                df.iloc[idx:idx + duration, 0] *= np.random.uniform(1.8, 3.0)  
                anomaly_labels[idx:idx + duration] = 1  
            elif atype == "dip":  
                duration = np.random.randint(1, 5)  
                df.iloc[idx:idx + duration, 0] *= np.random.uniform(0.1, 0.4)  
                anomaly_labels[idx:idx + duration] = 1  
            elif atype == "drift":  
                duration = np.random.randint(10, 30)  
                drift = np.linspace(0, np.random.uniform(30, 80), duration)  
                end_idx = min(idx + duration, n)  
                df.iloc[idx:end_idx, 0] += drift[:end_idx - idx]  
                anomaly_labels[idx:end_idx] = 1  
            elif atype == "outage":  
                duration = np.random.randint(5, 20)  
                end_idx = min(idx + duration, n)  
                df.iloc[idx:end_idx, 0] = 0.0  
                anomaly_labels[idx:end_idx] = 1
    ...
    def _freq_to_minutes(freq: str) -> int:  
        if "T" in freq or "min" in freq:  
            return int("".join(filter(str.isdigit, freq)) or "1")  
        elif "H" in freq:  
...
class GridGuardDataLoader:  
    ...
    def get_full_pipeline(self, source_override=None):  
